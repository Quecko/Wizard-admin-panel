...
function GameBanner() {
    ...
    const getNewsBanner = () => {
        setOpen(true)
        axios.post(Environment.backendUrl + "/banner/get/all", { category: 'other' }, { headers: { "Authorization": `Bearer ${token}` } })
            .then((response) => {
    ...
    const News = myFiles.map(elem => {
        return (
            <tr>
                ...
                {Acls?.otherBanner?.delete ?   <td> <button className="buttons-remove" type="button" onClick={() => { setElemId(elem.id); window.$('#exampleModal346').modal('show') }}><i className="far fa-trash-alt"></i></button></td>: ''}

                {Acls?.otherBanner?.update ?    <td> <Link to={'/admin/EditBanners/' + elem?.id}><i className="fas fa-edit"></i></Link></td> : ''}
                </tr>
        )
    })
                            ...
                            <table class="table ">
                                <thead>
                                    <tr>
                                        ...
                                        <th>Type</th>
                                        {/* <th>Link/Page</th> */}
                                        {Acls?.otherBanner?.delete ?  <th>Remove</th>: ''}
                                        {Acls?.otherBanner?.update ?  <th>Edit</th> : ''}
