...
function Coin() {
    ...
    const getNewsBanner = () => {
        setOpen(true)
        axios.get(Environment.backendUrl + "/coin/all", { headers: { "Authorization": `Bearer ${token}` } })
            .then((response) => {
    ...
    const News = myFiles.map(elem => {
        return (
            <tr>
                ...
                <td className="eleipiess">{elem.contractAddress}</td>
                <td> {Acls?.coins?.delete ? <button className="buttons-remove" type="button" onClick={() => collection(elem.id)}><i className="far fa-trash-alt"></i></button> : ''} </td>
                <td> {Acls?.coins?.update ? <Link to={'/admin/EditCoins/' + elem?.id}><i class="fas fa-edit"></i></Link> : '' }</td>
    ...
    const deleteCoin = () => {
        close()
        setOpen(true)
        axios.get(Environment.backendUrl + "/coin/delete/" + id, { headers: { "Authorization": `Bearer ${token}` } })
            .then((response) => {
                setOpen(false)
