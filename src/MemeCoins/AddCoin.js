...
console.log("coinsssss",inputs)
    ...
    const FIndcoin = () => {
        setOpen(true)
        axios.get(Environment.backendUrl + "/MemeTokens/find/" + id, { headers: { "Authorization": `Bearer ${token}` } })
            .then((response) => {
    ...
    const Addcoin = async (event) => {
        ...
        if (myFiles != '' && inputs.name != '' && inputs.symbol != '' && inputs.contractAddress != '') {
            axios.post(Environment.backendUrl + "/MemeTokens/create", data, { headers: { "Authorization": `Bearer ${token}`, 'Content-Type': 'multipart/form-data' } })
                .then((response) => {
                    setOpen(false)
    ...
    const editCoin = async (event) => {
        ...
        data.append("id", id)
        axios.post(Environment.backendUrl + "/MemeTokens/edit", data, { headers: { "Authorization": `Bearer ${token}`, 'Content-Type': 'multipart/form-data' } })
            .then((response) => {
                setOpen(false)
                setTimeout(() => {
                    toast.success(response.data.msg, {
                                        ...
                                        <div className="col-12">
                                            <div class="form-group">
                                                <div className="videopage">
                                                    {id ? <button className="red-b" onClick={editCoin}>Edit</button> : <button className="red-b" onClick={Addcoin}>Publish</button>}
